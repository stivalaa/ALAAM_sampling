...
#
# $Id: plotRMSE.R 648 2016-09-27 07:40:15Z stivalaa $
#
...
# RMSE
#
# Usage: Rscript plotRMSE.R baseline_Stats_table.txt  outfilename.eps
#
...
#
# Example: cat estimation_n500_bin_cont_*_error_statistics.txt | Rscript plotRMSE.R  estimation_n500_bin_cont_rmse.eps
# 
...
if ("percentRemoved" %in% colnames(D)) {
    ...
    p <- ggplot(D, aes(x = numNodes, y = RMSE))
    p <- p + xlab('Number of nodes in sample')

    p <- p + geom_point()
    p <- p + geom_errorbar(aes(ymax = RMSE_upper, ymin = RMSE_lower))
    ## p <- p + geom_hline(aes(yintercept = RMSE.baseline))
    ## p <- p + geom_hline(aes(yintercept = RMSE_lower.baseline), linetype="dashed")
    ## p <- p + geom_hline(aes(yintercept = RMSE_upper.baseline), linetype="dashed")
    p <- p + theme_bw()
...
#    p <- p + facet_wrap(~ Effect, scales="free_y")
    p <- p + facet_wrap(~ Effect, scales="free")
    ...
    for (wave in unique(D$numWaves)) {
        ...
        if (wave == 1) {
            ...
            D <- D[which(D$numSeeds >= 10), ] # XXX error bar on RMSE too large for small samples
        }
        maxNumSeeds <- max(D$numSeeds)    
        D$numSeeds <- as.factor(D$numSeeds)
        
        p <- ggplot(D, aes(x = numSeeds, y = RMSE,
...
#        p <- p + scale_linetype("m")
        ...
        p <- p + geom_point()
        p <- p + geom_errorbar(aes(ymax = RMSE_upper, ymin = RMSE_lower))
        p <- p + geom_hline(aes(yintercept = RMSE.baseline),colour="gray55")
        p <- p + geom_hline(aes(yintercept = RMSE_lower.baseline), linetype="dashed",colour="gray55")
        p <- p + geom_hline(aes(yintercept = RMSE_upper.baseline), linetype="dashed",colour="gray55")
