...
#
# read_attr_fie() - read attributes file in one-column format  
...
read_attr_file <- function(filename) {
    attr_df <- read.table(filename, header=F)
    return(attr_df$V1)
}
...
# write_outcome_file() - write attr file in parallel spnet (Pajek .clu) file format
#
# Parameters:
#    filename - filename to write to (warning: overwritten)
...
write_outcome_file <- function(filename, attrs) {
  f <- file(filename, open="wt")
  cat('*vertices ', length(attrs), '\n', file=f)
  write.table(attrs, file=f, row.names=F, col.names=F)
  close(f)
}
...
#    None.
#
write_attr_file <- function(filename, outcomes) {
  f <- file(filename, open="wt")
  write.table(outcomes, file=f, row.names=F, col.names=F)
  close(f)
}
