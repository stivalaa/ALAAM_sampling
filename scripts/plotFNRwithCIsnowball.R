#!/usr/bin/Rscript
#
# File:    plotFNRwithCIsnowball.R
# Author:  Alex Stivala
# Created: September 2015
#
# $Id: plotFNRwithCIsnowball.R 648 2016-09-27 07:40:15Z stivalaa $
#
...
#
# Usage: Rscript plotFNRwithCIsnowball.R  baseline_Stats_Table.txt outfilename.eps
#
...
#
# Example: cat estimation_n500_bin_cont2_snowball_full_error_statistics.txt | Rscript plotFNRwithCIsnowball.R estimation_n500_bin_cont_baseline_error_statistics.txt   estimation_n500_bin_cont2_snoball_full_false_negative_rate.eps
# 
...
for (i in 1:length(effects)) {
    ...
    p <- ggplot(De, aes(x = numSeeds, y = FNRpercent,
                        colour = as.factor(maxLinks),
#                        linetype = as.factor(maxLinks),
                        shape = as.factor(maxLinks)))
    p <- p + geom_hline(yintercept = ebaseline$FNRpercent,colour="gray55")
    p <- p + geom_hline(yintercept = ebaseline$FNRpercentLower, linetype="dashed",colour="gray55")
    p <- p + geom_hline(yintercept = ebaseline$FNRpercentUpper, linetype="dashed",colour="gray55")
    p <- p + geom_point()
    ...
    if (landscape) {
        p <- p + scale_x_discrete(breaks=seq(0, maxNumSeeds, 20))
    } else {
