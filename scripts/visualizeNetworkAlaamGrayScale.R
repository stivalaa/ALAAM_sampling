#!/usr/bin/Rscript
#
# File:    visualizeNetworkAlaamGrayScale.R
# Author:  Alex Stivala
...
#
#
# Usage: Rscript visualizeNetworkAlaamGrayScale.R [-d] graphfile.txt binattributesfile.txt continuousAttributsfile.txt outcomefile.txt output_postscript_file.eps
#
...
#
# Example usage:
#  Rscript visualizeNetworkAlaamGrayScale..R n500_kstar_simulate12750000_matrix.txt  binaryAttribute_50_50_n500.txt  continuousAttributes_n500.txt sample-n500_bin_cont4800000.clu n500_sample4800000.eps
#
...
if (length(args) < 5 || length(args) > 6) {
  cat("Usage: Rscript visualizeNetworkAlaamGrayScale..R [-d] graph_filename.txt binattr_filename.txt contattr_filename.txt outcomefilename.clu output_file.eps\n")
  quit(save="no")
}
basearg <- 0
directed = FALSE
...
palette <- rev(brewer.pal(max(binattr)+1, "Greys"))
